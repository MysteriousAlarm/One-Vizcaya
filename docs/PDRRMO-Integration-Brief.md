...
| **Photo evidence** | Rarely attached; hard to share quickly | Photo attached at capture with hardware-verified timestamp and location metadata |
...
| **False or recycled reports** | Hard to filter; waste dispatcher time | Photo EXIF metadata verified at capture; every report tied to a verified, identified resident — no anonymous submissions |
...
We are building a dedicated **Emergency Button** on the report submission
screen and the app home screen:

```
┌─────────────────────────────────────┐
│                                     │
│   🚨  EMERGENCY                     │
│   Tap to contact PDRRMO OpCen       │
│                                     │
│   [ CALL NOW ]   [ SEND LOCATION ]  │
│                                     │
└─────────────────────────────────────┘
```

- **CALL NOW** — one tap dials the PDRRMO Operations Center directly.
- **SEND LOCATION** — one tap fires a pre-filled emergency report with
  the user's current GPS coordinates to the PDRRMO priority queue,
  bypassing the normal category selection flow.

> **Honest status:** the Hotline Directory is fully implemented. The
> dedicated one-tap Emergency Button with the direct PDRRMO OpCen dial
> and location-push is **in active development** as a direct result of
> this committee's feedback. We will demo it at the next meeting.
> **[CONFIRM WITH PDRRMO]** the correct OpCen number and whether a
> separate SMS fallback is needed for residents without data signal.
